...
export function ExamCountdown() {
  ...
  const unhide = () => {
    setHidden(false);
    try { localStorage.setItem(COUNTDOWN_HIDDEN_KEY, "0"); } catch { /* ignore storage errors */ }
  };

  // Hidden -> collapse to a slim, reversible bar so it can be brought back anytime.
  if (hidden) {
    return (
      <section className="py-6 sm:py-8">
        <Container>
          <button
            type="button"
            onClick={unhide}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-[var(--radius-md)] border border-dashed border-[var(--color-border)] text-xs font-medium text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)] hover:border-[var(--color-border-strong)] transition-colors cursor-pointer"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
              <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" />
            </svg>
            Afișează numărătoarea până la examen
          </button>
        </Container>
      </section>
    );
    ...
    <section className="py-6 sm:py-8">
      <Container>
        <div className="relative overflow-hidden rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)] animate-slide-up">
          ...
          {/* Hide the countdown (reversible + remembered) for anyone it distracts;
              an eye-off toggle, brought back from the slim "show" bar above. */}
          <button
            type="button"
            onClick={hide}
            aria-label="Ascunde numărătoarea"
