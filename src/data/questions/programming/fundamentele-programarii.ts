...
2`,
    ...
    codeLanguage: "c",
    options: {
      a: `; Ce valori vor fi afișate pe ecran după executarea secvenței de mai sus? 2 a) 124 4`,
      b: `123 4`,
      c: `123 5`,
      d: `124 3`,
    },
...
return 0; 
    ...
    codeLanguage: "c",
    options: {
      a: `++; } int main() { int x=4, y=-2; f(x,&y); g(&x,y); printf("%d %d",x,y); return 0; } Ce valori se vor afişa pe ecran după executarea programului de mai sus? a) 4 8`,
      b: `8 8`,
      c: `8 6`,
      d: `6 6`,
    },
...
jos atribuie variabilei t de tip întreg valoarea expresiei a*b+a*b*c?`,
  ...
  },
  {
    ...
    options: {
      ...
      b: `int f(int n) { int s=0; while(n!=0) { s=s+n/10; n=n%10; } return s; 11 }`,
...
de numere întregi transmis ca parametru?`,
    ...
    codeLanguage: undefined,
    options: {
      a: `int suma(int v[],int n) 12 { int s=0,k=0; while(k<n) s+=v[k++]; return s; }`,
...
de cod afișează suma elementelor de pe fiecare linie a matricei 𝑎?`,
    ...
    codeLanguage: undefined,
    options: {
      a: `for(int i=0;i<n;i++) { int s=0; for(int j=0;j<n;j++) { s=s+a[i][j]; printf("%d ",s); } 15 }`,
...
cale este transmisă prin parametrul de intrare 𝑛𝑓?`,
    ...
    options: {
      ...
      c: `int nb(char *nf) { FILE *f=fopen(nf,"r"); int n=sizeof(f); fclose(f); 16 return n; }`,
...
este transmisă prin parametrul de intrare 𝑛𝑓 (se presupune că fișierul nu conține linii vide)?`,
    ...
    options: {
      ...
      d: `int nr(char *nf) { char s[1001]; FILE *f=fopen(nf,"r"); int n=0; while(fgets(s,1000,f)) n++; fclose (f); 17 return n; }`,
    },
    correctAnswer: "a",
  },
  {
...
𝑣 format din 𝑛 numere întregi sau -1 dacă tabloul nu conține nici un număr pozitiv:`,
    ...
    options: {
      ...
      d: `int p(int v[],int n) { int i; for(int i=0;i<n;i++) if (v[i]>0) return i; 19 else return -1; }`,
    },
    correctAnswer: "a",
  },
  {
